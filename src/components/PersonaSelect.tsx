...
import { X, User, ShieldCheck, MessageCircleHeart, Star, ArrowRight, Upload, FileText, Sparkles, Zap, Eye, Code2 } from "lucide-react";
      ...
      <div className="flex flex-col lg:flex-row min-h-screen">
        ...
        <div className="lg:w-1/2 flex items-center justify-center bg-card p-8 lg:p-16 lg:sticky lg:top-0 lg:h-screen relative overflow-hidden border-r border-border">
          ...
          >
            ...
            >
              <img src={heroCharacters} alt="FixUx characters" className="w-full h-auto max-h-[340px] object-contain" />
            </motion.div>
            ...
            >
              ...
              <p className="text-muted-foreground text-base lg:text-lg mt-3 leading-relaxed">
                AI-powered design audits & transcript-to-UI
                <br className="hidden sm:block" /> generation in one platform.
              </p>
            </motion.div>
          </motion.div>
        </div>
        ...
        {/* Right Column */}
        <div className="lg:w-1/2 flex flex-col items-center px-6 py-12 lg:px-12 lg:py-10 lg:overflow-y-auto bg-background">
          {/* Logo & Title */}
          <motion.div
          ...
          >
            ...
            <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight text-foreground mb-2 leading-[1.1]">
              What would you
              <br />
          ...
          {/* ═══ Two Feature Cards ═══ */}
          <div className="grid grid-cols-1 gap-4 max-w-lg w-full mb-8">
            {/* Feature 1: UX Audit */}
            <motion.button
              initial={{ y: 20, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              ...
              className="group relative w-full text-left border-2 border-border bg-card transition-all duration-300 overflow-hidden"
              style={{
                borderColor: activeFeature === "audit" ? "hsl(var(--primary))" : undefined,
                boxShadow: activeFeature === "audit" ? "0 0 0 1px hsl(var(--primary)), 0 8px 25px -8px hsl(var(--primary) / 0.2)" : undefined,
              }}
            >
              <div className="p-6">
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 shrink-0 flex items-center justify-center bg-primary/10 border border-primary/20">
                    <Upload className="w-6 h-6 text-primary" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="text-lg font-extrabold text-foreground">UX Design Audit</h3>
                    ...
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      Upload a screenshot and get AI-powered design feedback with issue mapping, severity scoring, and fix suggestions.
                    </p>
                  </div>
                ...
                <div className="flex flex-wrap gap-2 mt-4 ml-16">
                  {["60+ UX Principles", "Issue Pinpointing", "Fix Suggestions", "5 Personas"].map((tag) => (
                    <span key={tag} className="px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider border border-border bg-accent/50 text-muted-foreground">
                      {tag}
                    </span>
                  ))}
                </div>
            ...
            >
              <div className="p-6">
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 shrink-0 flex items-center justify-center border" style={{ background: "hsl(262 83% 58% / 0.1)", borderColor: "hsl(262 83% 58% / 0.2)" }}>
                    <FileText className="w-6 h-6" style={{ color: "hsl(262 83% 58%)" }} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="text-lg font-extrabold text-foreground">Transcript → UI</h3>
                    ...
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      Paste a meeting transcript and AI generates a production-ready UI with HTML, CSS, JS — plus actionable insights.
                    </p>
                  </div>
                  <ArrowRight className="w-5 h-5 text-muted-foreground/40 group-hover:text-[hsl(262_83%_58%)] transition-colors shrink-0 mt-1" />
                </div>

                {/* Feature highlights */}
                <div className="flex flex-wrap gap-2 mt-4 ml-16">
                  {["Live Preview", "Code Export", "AI Insights", "5 Roles"].map((tag) => (
                    <span key={tag} className="px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider border text-muted-foreground" style={{ borderColor: "hsl(262 83% 58% / 0.2)", background: "hsl(262 83% 58% / 0.05)" }}>
                      {tag}
                    </span>
                  ))}
                </div>
          ...
          <AnimatePresence>
            {activeFeature === "audit" && (
              ...
              >
                <div className="pb-2">
                  <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.15em] mb-3 flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {personas.map((persona, index) => (
                      <motion.button
                        ...
                        className={`group relative flex items-center gap-4 p-4 border border-border bg-card text-left transition-all duration-300 card-hover focus:outline-none focus:ring-2 focus:ring-primary/50 ${personaAccentBorder[persona.id]}`}
                      >
          ...
          >
            ...
            >
              <motion.div animate={{ rotate: [0, 360] }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }} className="absolute -top-4 -right-4 w-20 h-20 rounded-full bg-primary/5 blur-sm" />
              <motion.div animate={{ rotate: [360, 0] }} transition={{ duration: 15, repeat: Infinity, ease: "linear" }} className="absolute -bottom-6 -left-6 w-24 h-24 rounded-full bg-accent/10 blur-sm" />
              <div className="relative z-10 p-5 flex items-center gap-4">
                <div className="relative shrink-0">
                  <motion.div
                    animate={{ y: [0, -5, 0], rotate: [0, 5, -5, 0] }}
                    transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
                    className="w-12 h-12 rounded-xl bg-gradient-to-br from-primary/20 to-primary/10 flex items-center justify-center border border-primary/20"
                  >
                    <MessageCircleHeart className="w-6 h-6 text-primary" />
                  </motion.div>
                  <motion.div animate={{ scale: [1, 1.3, 1], opacity: [0.7, 1, 0.7] }} transition={{ duration: 2, repeat: Infinity, delay: 0.5 }} className="absolute -top-1 -right-1">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-foreground flex items-center gap-2">
                    ...
                    <motion.span animate={{ rotate: [0, 14, -8, 0] }} transition={{ duration: 1.5, repeat: Infinity, repeatDelay: 2 }} className="inline-block text-base">✨</motion.span>
                  </p>
                  <p className="text-xs text-muted-foreground mt-0.5 leading-relaxed">Your feedback helps us build a better Fix UX</p>
                </div>
                <motion.div animate={{ x: [0, 4, 0] }} transition={{ duration: 1.2, repeat: Infinity }} className="shrink-0">
