...
export default function RoomViewPage() {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
          ...
          {hasFigma ? (
            <div className="relative w-full h-full min-h-[60vh]">
              ...
              <div className="absolute top-3 right-3 flex gap-2 z-20">
                {figmaCommentMode && (
                  <div className="bg-primary text-primary-foreground px-3 py-1.5 rounded-full text-[10px] font-bold animate-pulse">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className="h-8 gap-1.5 text-xs bg-background/90 backdrop-blur-sm"
            ...
            {!isExpired && (
              <p className="text-[11px] text-muted-foreground mt-1.5">
                {hasImage
                  ...
                  : hasFigma
                  ? "Toggle Comment mode to place pins on the design."
                    ...
                    <div className="flex items-start gap-2.5">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5 mb-0.5">
                          ...
                          {isRectComment && (
                            <span className="text-[9px] bg-primary/10 text-primary px-1 py-0.5 rounded font-semibold">
                              Area
                            </span>
                          )}
                            ...
                            {replies.map((r) => (
                              <div key={r.id}>
                                ...
                                {(isOwner || r.reviewer_id === user?.id) && (
                                  <button className="text-[10px] font-medium text-destructive mt-1" onClick={(e) => { e.stopPropagation(); deleteComment(r.id); }}>
                                    Delete
                                  </button>
                                )}
                              </div>
      ...
      {/* Fullscreen Modal — Image or Figma */}
      <AnimatePresence>
        {isFullscreen && (
          <motion.div
            initial={{ opacity: 0 }}
            ...
            <AnimatePresence>
              {(pendingPin || pendingRect) && !isExpired && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: 10 }}
                  className="absolute bottom-6 left-1/4 right-1/4 bg-card border border-border rounded-xl shadow-2xl p-4 z-30"
                >
                  <div className="flex items-start gap-3">
                    <Avatar className="h-8 w-8 shrink-0">
                      <AvatarFallback className="text-[10px] bg-primary/10 text-primary font-bold">
